...
pub(crate) fn skip_whitespace(stream: &mut TokenStream) {
    skip_next_token_if(stream, |k| k == TokenKind::WHITESPACE);
}
...
pub(crate) fn skip_whitespace_and_newline(stream: &mut TokenStream) {
    skip_next_token_if(stream, |k| {
        vec![TokenKind::WHITESPACE, TokenKind::NEWLINE].contains(&k)
    });
}

pub(crate) fn get_next_token_if<'a, F: Fn(TokenKind) -> bool>(
...
pub(crate) fn skip_next_token_if<F: Fn(TokenKind) -> bool>(stream: &mut TokenStream, pred: F) {
    while let Some(current_token) = stream.peek_token() {
        if pred(current_token.kind()) {
            stream.next_token();
        } else {
